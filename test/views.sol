...
contract GiftOfMidasTest is Test {
    ...
    function testPublicSaleViews() public {
        assertTrue(giftOfMidas.getPublicSaleAddressRemainingMints(userOne) == 20);
        hoax(userOne);
        giftOfMidas.publicMint{value : 0.5 ether}(10);
        assertTrue(giftOfMidas.getPublicSaleAddressRemainingMints(userOne) == 10);
    }

    function testPreSaleViews() public {
        assertTrue(giftOfMidas.getPreSaleAddressRemainingMints(userOne) == 1);
        hoax(userOne);
        giftOfMidas.preSaleMint{value : 0.4 ether}(1, userOneMerkleProof);
        assertTrue(giftOfMidas.getPreSaleAddressRemainingMints(userOne) == 0);
    }
